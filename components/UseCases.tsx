        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-5" data-reveal-group>
          {useCases.map((uc, i) => (
            <div
              key={i}
              data-reveal-item
              className="rounded-3xl border border-[var(--border)] bg-[var(--bg)] p-7 transition-all hover:-translate-y-0.5 hover:border-[var(--border-strong)]"
            >
              {uc.vertical && (
                <div className="inline-block text-[10px] tracking-[2px] uppercase text-[var(--text-muted)] mb-2 border border-[var(--border)] px-2.5 py-0.5 rounded">
                  {uc.vertical}
                </div>
              )}
              <h3 className="text-[21px] font-medium tracking-tight mb-3">{uc.title}</h3>
